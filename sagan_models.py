...
class Self_Attn(nn.Module):
    ...
    def __init__(self, b_size, imsize, in_dim, activation):
        super(Self_Attn, self).__init__()  # Self_Attn(batch_size, int(self.imsize/4), 128, 'relu')
        self.b_size = b_size  # what is b_size?  batch_size, int(self.imsize/4), 128, 'relu'
        self.imsize = imsize  # what is imsize?
        self.in_dim = in_dim
...
class Generator(nn.Module):
    ...
    def forward(self, z):
        z = z.view(z.size(0), z.size(1), 1, 1)  # 32x128x1x1
        out=self.l1(z)  # spectral norm conv
        out=self.l2(out)  # spectral norm conv
        out=self.l3(out)  # spectral norm conv
