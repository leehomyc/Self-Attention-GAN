...
class Trainer(object):
    def __init__(self, data_loader, config):
        ...
        self.use_inception = config.use_inception
    ...
    def train(self):
        ...
        for step in range(start, self.total_step):
            ...
            if self.use_inception:
                up = nn.Upsample(size=(299, 299), mode='bilinear').type(
                    torch.cuda.FloatTensor)
                fake_images_up = up(fake_images)
                fake_images_up_inception = self.inception_model(fake_images_up)
                scores = F.softmax(fake_images_up_inception).data.cpu().numpy()
            ...
            if (step + 1) % self.log_step == 0:
                ...
                self.logger.scalar_summary('inception', inception_score, step)
                print(
                    "Elapsed [{}], G_step [{}/{}], D_step[{}/{}], d_out_real: {:.4f}, ave_gamma_l3: {:.4f}, ave_gamma_l4: {:.4f}, inception_score: {:.4f}".
            ...
            if (step + 1) % self.sample_step == 0:
                ...
                self.logger.image_summary('images', denorm(fake_images.data), step)
    ...
    def load_pretrained_model(self):
        ...
        print(
            'loaded trained models (step: {})..!'.format(self.pretrained_model))
